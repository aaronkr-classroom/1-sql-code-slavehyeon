...
3. 몽고DB는 별도의 스키마 선언 없이도 릴레이션에 해당하는 컬렉션을 생성할 수 있다. (X)
...
4. (repalceOne)() 명령어 함수는 특정 문서를 찾아서 새로운 문서로 교체한다.
...
2. db.회원.updateMany({}, { $set: { 탈퇴날짜: new Date() } })
3. db.회원.drop()
...
Key-Value형	단순한 키-값 쌍, 매우 빠른 읽기/쓰기 (예: Redis)
문서형(Document)	JSON/BSON 문서 저장, 유연한 구조 (예: MongoDB)
컬럼형(Column)	열 기반 저장, 대용량 처리에 유리 (예: Cassandra)
그래프형(Graph)	노드/엣지 기반 관계 저장, 관계 분석에 유리 (예: Neo4j)

**11 몽고DB와 관계형 데이터베이스를 비교 • 설명하시오.**


데이터 모델	문서 기반(JSON/BSON)	테이블 기반
스키마	유동적(Dynamic Schema)	고정 스키마(Fixed Schema)
확장성	수평 확장(Sharding) 유리	수직 확장 중심
관계 처리	Join 미흡, 중첩 문서 사용	다양한 Join 및 제약조건 지원
적합한 데이터 형태	비정형/반정형 데이터	정형 데이터 중심
